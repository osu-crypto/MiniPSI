...
namespace tests_libOTe
{
	...
	int Ristretto_Test_Impl()
	{
		...
		randombytes_buf(px, sizeof px);
		std::cout << toBlock((u8*)&px ) << "\t px\n";
		...
		std::cout << toBlock((u8*)&fx) << "\t px^k\n";

		unsigned char pxk[crypto_core_ristretto255_BYTES];
		if (crypto_scalarmult_ristretto255(pxk, k, px) != 0) {
			return -1;
		}
		std::cout << toBlock((u8*)&pxk) << "\t px^k\n";
